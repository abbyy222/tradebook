...
import type { StockItemDTO } from '@tradebook/shared-types'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end animate-fade-in" style={{ background: 'rgba(10,5,2,0.8)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div className="w-full max-w-lg mx-auto rounded-t-3xl px-6 pt-5 flex flex-col gap-5 animate-slide-up" style={{ background: '#1e1208', border: '1px solid rgba(255,255,255,0.08)', borderBottom: 'none', paddingBottom: 'calc(2.5rem + env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3"><div className="flex flex-col gap-2"><label className="label-base">Qty in stock</label><input type="number" inputMode="numeric" placeholder="0" value={quantity} onChange={e => setQuantity(e.target.value)} className="input-base" /></div><div className="flex flex-col gap-2"><label className="label-base">Unit price (N)</label><input type="number" inputMode="decimal" placeholder="0" value={unitPrice} onChange={e => setUnitPrice(e.target.value)} className="input-base" /></div></div>
        <div className="flex flex-col gap-2"><label className="label-base">Low stock alert at</label><input type="number" inputMode="numeric" placeholder="5" value={threshold} onChange={e => setThreshold(e.target.value)} className="input-base" /><p className="font-body text-xs" style={{ color: 'rgba(245,237,224,0.3)' }}>Alert you when quantity falls below this</p></div>
        <button onClick={() => createStockItem.mutate({ itemName: itemName.trim(), quantity: parseInt(quantity, 10), unitPrice: parseFloat(unitPrice), lowStockThreshold: parseInt(threshold, 10) }, { onSuccess: onClose })} disabled={!itemName.trim() || !quantity || !unitPrice || createStockItem.isPending} className="btn-primary mt-2">{createStockItem.isPending ? <span className="rounded-full border-2 border-white/30 border-t-white" style={{ width: 20, height: 20, animation: 'spin 0.7s linear infinite' }} /> : 'Add item'}</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen">
      <div className="relative overflow-hidden px-5 pt-12 pb-6" style={{ background: 'linear-gradient(180deg, rgba(78,204,163,0.1) 0%, transparent 100%)' }}>
        <div className="relative z-10 flex items-center justify-between max-w-lg mx-auto"><div><p className="label-base mb-0.5">Inventory</p><h1 className="font-display font-bold" style={{ fontSize: '1.75rem', letterSpacing: '-0.02em', color: '#f5ede0', fontVariationSettings: "'WONK' 1, 'opsz' 30" }}>Stock</h1></div><button onClick={() => setAddOpen(true)} className="rounded-xl px-4 py-2.5 font-ui font-bold text-sm" style={{ background: 'linear-gradient(135deg, #c04818, #e8a838)', color: '#fff' }}>+ Add</button></div>
      </div>
