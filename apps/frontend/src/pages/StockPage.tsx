...
const commonUnitOptions = ['piece', 'pack', 'carton', 'bag', 'bottle', 'crate', 'dozen', 'kg', 'g', 'litre', 'ml', 'roll']
const unitSuggestionsId = 'stock-unit-suggestions'
...
const SummaryCard = ({ label, value, sublabel }: { label: string; value: string; sublabel: string }) => (
  <div className="rounded-2xl px-4 py-4" style={{ background: '#231510', border: '1px solid rgba(255,255,255,0.05)' }}>
    ...
    <p className="mt-1 font-body text-xs" style={{ color: 'rgba(245,237,224,0.35)' }}>{sublabel}</p>
  </div>
)
              ...
              return (
                <div key={movement.id} className="rounded-2xl px-4 py-4" style={{ background: '#231510', border: '1px solid rgba(255,255,255,0.08)' }}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-ui text-xs font-bold uppercase tracking-[0.08em]" style={{ color: meta.color }}>{meta.label}</p>
                      ...
                      {movement.actorTraderName ? (
                        <p className="mt-1 text-[11px] text-secondary">By {movement.actorTraderName}</p>
                      ) : null}
                    </div>
                    <div className="text-right">
                      <p className="font-display font-bold" style={{ color: movement.quantityDelta >= 0 ? '#4ecca3' : '#f87171' }}>
                        {quantityLabel} {movement.unitName}
                      </p>
                      <p className="mt-1 text-[11px] text-secondary">
                        After: {movement.quantityAfter != null ? formatQuantityWithUnit(movement.quantityAfter, movement.unitName) : '-'}
                      </p>
                    </div>
                  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end animate-fade-in" style={{ background: 'rgba(10,5,2,0.8)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div className="mx-auto flex max-h-[92vh] w-full max-w-lg flex-col gap-5 overflow-y-auto rounded-t-3xl px-4 pt-4 sm:px-6 sm:pt-5 animate-slide-up" style={{ background: '#1e1208', border: '1px solid rgba(255,255,255,0.08)', borderBottom: 'none', paddingBottom: 'calc(2.5rem + env(safe-area-inset-bottom))' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            <label className="label-base">Quantity unit</label>
            <input type="text" list={unitSuggestionsId} placeholder="piece" value={unitName} onChange={(e) => setUnitName(e.target.value)} className="input-base" />
          </div>
        </div>
        ...
        <p className="font-body text-xs" style={{ color: 'rgba(245,237,224,0.35)' }}>
          Use any stock unit that fits the business, like piece, bag, carton, bottle, kg, or a custom name. Cost price keeps inventory value honest, while wholesale stays separate for bulk pricing.
        </p>

        <datalist id={unitSuggestionsId}>
        ...
        <button
          onClick={submit}
          disabled={!itemName.trim() || !quantity || !unitName.trim() || !unitPrice || !costPrice || hasWholesalePairMismatch || createStockItem.isPending}
    ...
    <div className="fixed inset-0 z-50 flex items-end animate-fade-in" style={{ background: 'rgba(10,5,2,0.8)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div className="mx-auto flex max-h-[92vh] w-full max-w-lg flex-col gap-5 overflow-y-auto rounded-t-3xl px-4 pt-4 sm:px-6 sm:pt-5 animate-slide-up" style={{ background: '#1e1208', border: '1px solid rgba(255,255,255,0.08)', borderBottom: 'none', paddingBottom: 'calc(2.5rem + env(safe-area-inset-bottom))' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div>
          ...
          <p className="mt-1 font-body text-sm" style={{ color: 'rgba(245,237,224,0.4)' }}>
            {item.itemName} currently has <span style={{ color: '#e8a838' }}>{formatQuantityWithUnit(item.quantity, item.unitName)}</span>.
          </p>
        </div>
        ...
        {!quantity.trim() ? (
          <p className="font-body text-xs" style={{ color: 'rgba(245,237,224,0.35)' }}>Leave quantity at 0 if you only want to update the unit name, selling, cost, wholesale, or low-stock values.</p>
        ) : null}
        ...
        <button
          onClick={submit}
          disabled={(!quantity.trim() && unitName.trim() === item.unitName && unitPrice === String(item.unitPrice) && costPrice === String(item.costPrice) && wholesalePrice === (item.wholesalePrice == null ? '' : String(item.wholesalePrice)) && wholesaleMinQty === (item.wholesaleMinQty == null ? '' : String(item.wholesaleMinQty)) && lowStockThreshold === String(item.lowStockThreshold)) || (quantity.trim() && parseInt(quantity, 10) < 0) || !unitName.trim() || !Number.isFinite(parsedUnitPrice) || parsedUnitPrice <= 0 || !Number.isFinite(parsedCostPrice) || parsedCostPrice < 0 || hasWholesalePairMismatch || !Number.isFinite(parsedThreshold) || parsedThreshold < 0 || adjustStock.isPending}
...
export const StockPage = () => {
  ...
  return (
    <div className="min-h-screen">
      <div className="px-4 pb-6 pt-8 sm:px-5 sm:pt-10">
        <div className="mx-auto max-w-6xl">
          <MarketSceneBanner
            ...
            eyebrow="Inventory"
            title="Stock"
            description="Cost-aware inventory for valuation, retail sales, and flexible quantity units that fit the real way you trade."
            badge="Goods in hand"
          >
            {isOwner ? (
              <button onClick={() => setAddOpen(true)} className="shrink-0 rounded-xl px-4 py-2.5 font-ui text-sm font-bold" style={{ background: 'linear-gradient(135deg, #c04818, #e8a838)', color: '#fff' }}>+ Add</button>
            ) : null}
          </MarketSceneBanner>
        </div>
      </div>

      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 pb-10 sm:px-5">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <SummaryCard label="Units left" value={summary.units.toLocaleString('en-NG')} sublabel="Total quantity recorded across all stock items" />
        </div>

        <div className="relative">
          <svg className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2" width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ color: 'rgba(245,237,224,0.25)' }}>
        ...
        {isLoading ? (
          ...
          <div className="relative overflow-hidden rounded-[26px] border border-white/10 px-5 py-12 text-center" style={{ background: '#231510' }}>
            <div className="absolute inset-0 bg-cover bg-center opacity-20" style={{ backgroundImage: "linear-gradient(180deg, rgba(20,13,10,0.62) 0%, rgba(20,13,10,0.86) 100%), url('/market-scenes/dashboard-market-1.jpg')" }} />
            ...
            <div className="relative flex flex-col items-center gap-3">
              ...
              <p className="font-display text-lg font-bold" style={{ color: '#f5ede0', fontVariationSettings: "'WONK' 1" }}>{search ? 'No items found' : 'No stock yet'}</p>
              <p className="max-w-sm font-body text-sm" style={{ color: 'rgba(245,237,224,0.42)' }}>
                {search ? 'Try another item name or clear the search.' : 'Start with your first item so TradeBook can watch quantity, units, stock value, and expected margin.'}
              </p>
              {!search && isOwner ? (
                <button onClick={() => setAddOpen(true)} className="mt-2 rounded-xl px-4 py-2.5 font-ui text-sm font-bold" style={{ background: 'linear-gradient(135deg, #c04818, #e8a838)', color: '#fff' }}>
                  Add first item
                </button>
              ) : null}
            </div>
          ...
          const wholesaleLabel = item.wholesalePrice != null && item.wholesaleMinQty != null
            ? `Wholesale ${fmt(item.wholesalePrice)} from ${item.wholesaleMinQty} ${item.unitName} · `
            : ''
          const pricingSummary = `Sell ${fmt(item.unitPrice)} per ${item.unitName} · Cost ${fmt(item.costPrice)}`
          const valuationSummary = `${wholesaleLabel}Stock value ${fmt(item.stockValue)} · Margin room ${fmt(item.expectedGrossProfit)}`

          return (
            <div key={item.id} className="flex flex-col items-stretch gap-3 rounded-2xl px-4 py-4 sm:flex-row sm:items-center sm:gap-4 sm:px-5" style={{ background: '#231510', border: `1px solid ${isLow ? 'rgba(226,75,74,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
              <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl font-mono text-[0.8rem] font-semibold" style={{ background: isLow ? 'rgba(226,75,74,0.12)' : 'rgba(78,204,163,0.08)', color: isLow ? '#f87171' : '#4ecca3' }}>
                {item.quantity}
              </div>

              <div className="min-w-0 flex-1">
                <p className="truncate font-ui text-sm font-semibold" style={{ color: '#f5ede0' }}>{item.itemName}</p>
                <p className="mt-1 text-xs text-secondary">{formatQuantityWithUnit(item.quantity, item.unitName)} in stock</p>
                <p className="mt-0.5 font-body text-xs" style={{ color: 'rgba(245,237,224,0.35)' }}>
                  {pricingSummary}
                  {isLow && <span style={{ color: '#f87171', marginLeft: '0.5rem' }}> · Low stock!</span>}
                </p>
                <p className="mt-1 font-body text-xs" style={{ color: 'rgba(245,237,224,0.28)' }}>
                  {valuationSummary}
                </p>
              </div>
              ...
              <div className="flex flex-col items-stretch gap-2 sm:items-end">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex">
                  <button onClick={() => setHistoryItem(item)} className="rounded-full px-3 py-1 font-ui text-xs font-bold" style={{ background: 'rgba(159,176,255,0.12)', color: '#9fb0ff', border: '1px solid rgba(159,176,255,0.2)' }}>History</button>
                  {isOwner ? <button onClick={() => setSelectedItem(item)} className="rounded-full px-3 py-1 font-ui text-xs font-bold" style={{ background: 'rgba(78,204,163,0.12)', color: '#4ecca3', border: '1px solid rgba(78,204,163,0.2)' }}>Adjust</button> : null}
                </div>
              </div>
            </div>
          )
      ...
      {isOwner && addOpen ? <AddStockSheet onClose={() => setAddOpen(false)} /> : null}
      {isOwner && selectedItem ? <AdjustStockSheet item={selectedItem} onClose={() => setSelectedItem(null)} /> : null}
      {historyItem ? <StockHistorySheet item={historyItem} onClose={() => setHistoryItem(null)} /> : null}
    </div>
  )
}
